...
/**
 * Created by zw on 2017/6/21.
 */

public class IPTVPlayerBase extends IPTVPlayer implements IPlayerInterface{
    private static final String TAG = "IPTVPlayerBase";
    ...
    private void initSurface() {
        ...
        if (surfaceView != null) {
            ...
            surfaceHolder.addCallback(new IPTVPlayerBase.SurfaceViewHolder());
        }
    }
    ...
    public int joinChannel(int userChannelID) {
        ...
        if (mCurrentLive != null) {
            ...
            if ((!TextUtils.isEmpty(mCurrentLive.getChannelFCCIP()) && !"0".equalsIgnoreCase(mCurrentLive.getChannelFCCPort()))) {
                ...
                NvSetFCCParam(mCurrentLive.getUserChannelID(), mCurrentLive.getChannelFCCIP(), Integer.parseInt(mCurrentLive.getChannelFCCPort()));
            }
            NvLoadAndPlay(mCurrentLive.getChannelURL(), mCurrentLive.getTimeShiftURL(), true);
...
//		}
    ...
    public boolean isPause() {
        boolean state = true;
        state = (getPlayState() == STATE_PAUSE);
...
//		}
    ...
    public void playByTime(int type, String timestamp) {
        ...
        if (type == 1) {
            NvSeek(Integer.parseInt(timestamp));
        } else {
            NvLiveSeek(timestamp);
        }
...
//			}
    ...
    public void switchAudioChannel() {
        ...
        NvSetAudioBalance(audioChannel);
    ...
    public void switchAudioTrack(int id) {
        if (id >= 0) {
            ...
            NvSwitchAudioTrack(id);
            return;
        }

        String audioPIDs = "";
        audioPIDs = NvGetAudioPIDs();
        int pid = 0;
    ...
    public String getAllAudioTrackInfo() {
        ...
        String audioPIDs = NvGetAudioPIDs().trim();
    ...
    public String getCurrentAudioChannel() {
        String result = "";
        int audioBalance = NvGetAudioBalance();
        if (audioBalance == 1)
            result = "Left";// 左声道
    ...
    public boolean getShiftStatus() {
        return NvGetShiftStatus();
    }

    public String getCurrentTimeStr() {
        return NvGetCurrentTimeStr();

    }

    public int getCurrentTime() {
        return NvGetCurrentTime();
    }

    public int getCurrentTimeMs() {
        return NvGetCurrentTimeMs();
    }
    ...
    public void setVideoWindow(int x, int y, int w, int h) {
        NvSetVideoWindow(x, y, w, h);
    }

    public static int getVideoWidthPixels() {
        int videoWitdhPixels = NvGetVideoWidthPixels();
        ALOG.info("getVideoWidthPixels > "+videoWitdhPixels);
        return videoWitdhPixels;
    }

    public static int getVideoHeightPixels() {
        int videoHeightPixels = NvGetVideoHeightPixels();
        ALOG.info("getVideoHeigthPixels > "+videoHeightPixels);
    ...
    private boolean setSurface(Surface surface) {
        if (android.os.Build.VERSION.SDK_INT > 18) {
            return NvSetSurface(surface, "mNativeObject");
        } else if (android.os.Build.VERSION.SDK_INT >= 13)
            return NvSetSurface(surface, "mNativeSurface");
        else
            return NvSetSurface(surface, "mSurface");
    }
